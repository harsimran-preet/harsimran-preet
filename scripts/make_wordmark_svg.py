"""
Render a name as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
SVG that animates on GitHub (SMIL only -- GitHub runs SVG animations in <img>,
but never JS).

Design + technique by Avi Vashishta (github.com/AVIVASHISHTA29), documented at
avivashishta.com/blog/3d-ascii-wordmark-svg-github-readme. Adapted here for
HARSIMRAN (9 letters -> COLS widened to 108 so each letter reads, per his doc).
...
# ---- geometry / grid ------------------------------------------------------
COLS = int(os.environ.get("WORDMARK_COLS", 108))
...
TEXT = os.environ.get("WORDMARK_TEXT", "HARSIMRAN")

MASK_H = int(os.environ.get("WORDMARK_MASK_H", 300))   # glyph raster height in mask px (drives voxel density)
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">harsimran@github: ~$ ./wordmark.sh --3d</text>')
