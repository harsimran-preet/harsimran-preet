...
# Portrait style (monochrome terminal window, types-in via SMIL) after
# Avi Vashishta's profile, github.com/AVIVASHISHTA29. Background removal here
# uses a local numpy matte (prep_photo.py) instead of his rembg+CLAHE pipeline.
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">harsimran@github: ~$ ./portrait.sh</text>')
...
status_prompt = "harsimran@github:~$ whoami "
status_name = "Harsimran Preet Singh"
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{status_prompt}<tspan fill="{INK}">{status_name}</tspan></text>')
# blinking cursor parked just past the name (13px monospace advance ~= 7.3px/char)
cur_x = PAD + int(round(len(status_prompt + status_name) * 7.3)) + 4
